  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-end gap-2">
        <button onClick={(e)=>{e.stopPropagation(); toggleFav(p);}} aria-label="toggle-fav" className="p-1">
          {isFav ? '💖' : '🤍'}
        </button>
        <div className="text-xs text-slate-400 group-hover:text-slate-600">Details →</div>
      </div>
    </div>
  );
}
          ...
          >
            <div className="flex justify-between items-start gap-4">
              <div className="flex gap-4 items-center">
                <img
                  src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${shortId(pokemon.url)}.png`}
                  alt={pokemon.name}
                  className="w-28 h-28 object-contain"
                />
                <div>
                  <h2 className="text-2xl font-bold capitalize">{pokemon.name}</h2>
                  <div className="text-sm text-slate-500">#{shortId(pokemon.url).padStart(3, '0')}</div>
                  <div className="mt-2 flex gap-2">
                    {details?.types?.map((t) => (
                      <span key={t.type.name} className="px-2 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-xs capitalize">{t.type.name}</span>
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {loading ? <p className="text-sm text-slate-500">Loading moves…</p> : (
                  <div className="mt-2 flex flex-wrap gap-2">
                    {details?.moves?.slice(0,8).map(m => (
                      <span key={m.move.name} className="px-2 py-1 rounded bg-slate-100 dark:bg-slate-800 text-xs capitalize">{m.move.name}</span>
                    ))}
                  </div>
                )}
                ...
                <div className="mt-4">
                  ...
                  {!loading && details && (
                    <div className="text-sm text-slate-600 mt-1">{details.height / 10} m · {details.weight / 10} kg</div>
                  )}
                </div>
              </div>
            </div>
...
function MainApp() {
  ...
  const [favorites, setFavorites] = useState(() => {
    try { return JSON.parse(localStorage.getItem('pokedex:favs') || '[]'); } catch { return []; }
  });

  useEffect(()=>{ localStorage.setItem('pokedex:favs', JSON.stringify(favorites)); }, [favorites]);
  ...
  const visible = useMemo(() => {
    ...
    if (filterType !== 'all') {
      arr = arr.filter(p => {
        ...
        try { const types = JSON.parse(saved); return types.includes(filterType); } catch { return true; }
      });
    }
  ...
  useEffect(() => {
    ...
    toPrefetch.forEach(p => {
      ...
      fetchPokemonDetails(p.url).then(d => {
  ...
  async function openDetails(p) {
    ...
    // optimistic prefetch
    fetchPokemonDetails(p.url).then(d => {
      try { localStorage.setItem(`pokedex:details:${shortId(p.url)}`, JSON.stringify(d)); } catch {}
    }).catch(()=>{});
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-50 to-white dark:from-slate-900 dark:to-slate-800 text-slate-900 dark:text-slate-100 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-3xl font-extrabold">Poke<span className="text-sky-500">Dex</span></h1>
            <p className="text-sm text-slate-500">Browse our curated collection of fan-favorite Pokémon.</p>
          </div>

          <div className="w-full md:w-1/2 flex gap-2">
            <input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search by Name" className="flex-1 px-3 py-2 rounded-lg border focus:ring focus:ring-sky-200" />
            <select value={filterType} onChange={(e)=>setFilterType(e.target.value)} className="px-3 py-2 rounded-lg border dark:bg-slate-900 ">
            ...
            <select value={sortBy} onChange={(e)=>setSortBy(e.target.value)} className="px-3 py-2 rounded-lg border dark:bg-slate-900">
        ...
        <main>
          <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {visible.map(p => (
              <PokemonCard key={p.url} p={p} onOpen={openDetails} isFav={!!favorites.find(f=>f.id===shortId(p.url))} toggleFav={toggleFav} />
            ))}
          </section>

          <div className="mt-4 flex justify-center">
            {isLoading && <div className="text-slate-500">Loading pokemons…</div>}
            {isError && <div className="text-red-500">Failed to load.</div>}
          </div>

          <div ref={loadMoreRef} className="mt-6 flex justify-center">
            {isFetchingNextPage ? <div className="text-slate-500">Loading more…</div> : hasNextPage ? <div className="text-slate-400 text-sm">Scroll down to load more</div> : <div className="text-slate-400 text-sm">No more pokemons</div>}
          </div>
        ...
        <aside className="mt-8">
          ...
          <div className="mt-2 flex gap-2 overflow-x-auto">
            {favorites.length===0 ? <div className="text-sm text-slate-500">No favorites yet — tap the heart on a card.</div> : favorites.map(f => (
              <div key={f.id} className="min-w-[120px] p-2 bg-white/50 dark:bg-slate-900/50 rounded-lg border flex items-center gap-2">
                <img src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${f.id}.png`} className="w-10 h-10" />
